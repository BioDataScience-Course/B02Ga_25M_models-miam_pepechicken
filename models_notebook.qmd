...
```{r chart1Alexia}
chart(data = lezards, circbasequeue ~ longtot %col=% sexe_rec) +
  geom_point()+ 
...
```{r chart3Jeanne}
chart(data=lezards, circbasequeue ~ masse_rec_rec %fill=% masse_rec_rec) +
...
#a <- chart(data = lezards, ~ largcrane %fill=% sexe_rec) +
#  geom_histogram() 

#b <- chart(data = lezards, ~ longcrane %fill=% sexe_rec) +
#  geom_histogram()
