...
```{r chart1Alexia}
chart(data = lezards, circbasequeue ~ longqueue %col=% sexe_rec) +
  geom_point()+ 
...
```{r chart2Camille}
A <- chart(data = lezards, longtot ~ masse %fill=% sexe_rec) +
  geom_boxplot() +
  labs(fill = "Sexe") +
  scale_fill_brewer(palette = "Accent")

B <- chart(data = lezards, longsansqueue ~ masse %fill=% sexe_rec) +
  geom_boxplot() +
  labs(fill = "Sexe") +
  scale_fill_brewer(palette = "Accent")
...
```{r chart3Jeanne}
chart(data=lezards, longqueue ~ masse_rec_rec %fill=% masse_rec_rec) +
