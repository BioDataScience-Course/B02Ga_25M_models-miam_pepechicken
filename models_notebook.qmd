...
```{r regressionlineaire1_1}
lezards_rl1_1 <- lm(data= lezards, circbasequeue ~ masse * sexe_rec)
tabularise(summary_(lezards_rl1_1))
```
...
```{r regressionlineaire1_2}
lezards_rl1_2 <- lm(data= lezards, circbasequeue ~ masse + sexe_rec)
tabularise(summary_(lezards_rl1_2))
```
...
```{r regressionlineaire1residus}
chart$residuals(lezards_rl1_2)
```
...
$$
`r eq__(lezards_rl1_2, use_coefs = TRUE, coefs_digits = 3)`
$$
...
```{r regressionlineaire2graphique}
...
```{r regressionlineaire2}
lezards_rl2 <- lm(data = lezards, longcrane ~ largcrane)
tabularise(summary_(lezards_rl2))
```
...
```{r regressionlineaire2residus}
chart$residuals(lezards_rl2)
```
...
$$
`r eq__(lezards_rl2, use_coefs = TRUE, coefs_digits = 3)`
$$
...
```{r modelelineaire1_2}
lezards_ml1_2 <- lm(data= lezards, circbasequeue ~ masse + longqueue + sexe_rec)
tabularise(summary_(lezards_ml1_2))
```
...
```{r modelelineaire1residus}
chart$residuals(lezards_ml1_2)
```
...
$$
`r eq__(lezards_ml1_2, use_coefs = TRUE, coefs_digits = 3)`
$$
...
```{r modelelineaire2_1}
lezards_ml2_1 <- lm(data = lezards, largcrane ~ longcrane * sexe_rec)
tabularise(summary_(lezards_ml2_1))
```

Le paramètre $\beta_1$ est significativement différent de 0 au seuil $\alpha$ de 5%. Ce modèle est donc trop complexe. Cependant, essayons de voir si le sexe n'aurait pas une interaction plus interréssante avec largeur du crâne à la place de la longueur.

```{r modelelineaire2_2}
lezards_ml2_2 <- lm(data = lezards, longcrane ~ largcrane * sexe_rec)
tabularise(summary_(lezards_ml2_2))
```

Pour ce modèle, tous les paramètres sont significativement différents de zéro au seuil $\alpha$ de 5%. Le $R^2$ est d'environ 0,77, très faiblement supérieur au précédent (0,76) mais traduis tout de même d'un bon modèle. Il en est de même pour la valeur de p, étant inférieure à 0,05, indiquant que le modèle est significatif.

```{r modelelineaire2residus}
chart$residuals(lezards_ml2_2)
```
...
$$
`r eq__(lezards_ml2_2, use_coefs = TRUE, coefs_digits = 3)`
$$
...
```{r modelelineairegeneralise1_2}
lezards_mlg1_2 <- glm(data = lezards, sexe_rec ~ cbq_lt + lq_lt + masse, family=binomial)
...
```{r modelelineairegeneralise1_3}
lezards_mlg1_3 <- glm(data = lezards, sexe_rec ~ cbq_lt + lq_lt, family=binomial)
tabularise(summary_(lezards_mlg1_3))
```
...
$$
`r eq__(lezards_mlg1_3, use_coefs = TRUE, coefs_digits = 3)`
$$
...
```{r modelelineairegeneralise2_2}
lezards_mlg2_2 <- glm(data = lezards, sexe_rec ~ largcrane * longcrane, family = binomial)
tabularise(summary_(lezards_mlg2_2))
```
...
$$
`r eq__(lezards_mlg2_2, use_coefs = TRUE, coefs_digits = 3)`
$$

```{r AIC}
AIC(lezards_rl1_2, lezards_rl2, lezards_ml1_2, lezards_ml2_2, lezards_mlg1_3, lezards_mlg2_2)
```

Selon le critère d'Akaike, les trois meilleurs modèles sont la régression linéaire 2, le modèle linéaire généralisé 2 et le modèle linéaire généralisé 2.
...
Enfin, nous avons décidé de conserver les 2 modèles linéaires généralisés car ceux-ci sont les plus pertinents. En effet, ces deux modèles sont les plus adéquats à analyser plus en profondeur car ils permettent de comprendre quels sont les paramètres qui sont les plus adéquats pour le sexage des varans.
