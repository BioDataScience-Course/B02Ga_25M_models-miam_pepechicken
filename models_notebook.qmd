...
$$
`r eq__(lezards_lm1_1, use_coefs = TRUE, coefs_digits = 3)`
$$
...
$$
`r eq__(lezards_ml1_1, use_coefs = TRUE, coefs_digits = 3)`
$$
...
$$
`r eq__(lezards_ml2_1, use_coefs = TRUE, coefs_digits = 3)`
$$
...
lezards_mlg1 <- glm(data = lezards, masse ~ (cbq_lt + lq_lt)*sexe_rec, family=gaussian)
...
```{r}
lezards_mlg1_1 <- glm(data = lezards, sexe_rec ~ cbq_lt + lq_lt + masse, family=binomial)
tabularise(summary_(lezards_mlg1))
```
...
```{r AIC}
AIC(lezards_lm1_1, lezards_lm2, lezards_ml1_1, lezards_ml2, lezards_mlg1_2, lezards_mlg2_1)
```
